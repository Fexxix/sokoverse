...
export default function SokobanArchitectPage() {
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
          <div className="text-center mb-8">
            ...
            <p className="font-mono mb-6 font-medium text-foreground/90 text-center max-w-md">
              You need to be logged in to access the level creator. This ensures
              your brilliant designs can be saved and shared!
            </p>
          </div>
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <p className="font-mono mb-6 font-medium text-foreground/90 text-center max-w-md">
          Our level editor is still being built! Soon you&apos;ll be able to
          create and share your own fiendish puzzles.
        </p>
        ...
        <div className="w-48 h-48 grid grid-cols-6 grid-rows-6 gap-1 my-8 pixelated-border p-2 bg-background/50">
          ...
          {[...Array(36)].map((_, i) => {
            ...
            const isWall = [
              0, 1, 2, 3, 4, 5, 6, 11, 12, 17, 18, 23, 24, 29, 30, 31, 32, 33,
              34, 35,
            ].includes(i)
            ...
            if (isEditorCursor) {
              return (
                <div
                  key={i}
                  className="bg-transparent border-2 border-primary animate-pulse"
                />
              )
            } else if (isWall) {
              return (
                <div
                  key={i}
                  className="bg-primary/70 border border-primary/30"
                />
              )
            } else if (isBox) {
              return (
                <div
                  key={i}
                  className="bg-yellow-600/70 border border-yellow-600"
                />
              )
            } else if (isTarget) {
              return (
                <div
                  key={i}
                  className="bg-transparent border-2 border-dashed border-primary"
                />
              )
            } else if (isPlayer) {
              return (
                <div key={i} className="bg-primary border border-primary" />
              )
            } else {
              return (
                <div
